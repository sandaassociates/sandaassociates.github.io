...
function toggleMenu() {
    ...
    // Computed style check fixes the "double-click required" bug
    const currentDisplay = window.getComputedStyle(links).display;

    if (currentDisplay === 'flex') {
        links.style.display = 'none';
    } else {
        links.style.cssText = 'display:flex;flex-direction:column;position:fixed;top:72px;left:0;right:0;background:rgba(6,14,28,0.98);padding:20px;gap:16px;border-bottom:1px solid rgba(201,162,87,0.2);z-index:999;';
